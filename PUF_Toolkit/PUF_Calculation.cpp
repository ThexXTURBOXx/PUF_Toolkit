...
//global variable to retain the jaccard index value between function calls
static float jindex = 0;
static float frd_jindex = 0;
        ...
        for(it = list_of_files.begin();it != list_of_files.end(); ++it){
            ...
            if(error == 0){
                ...
                if (result_file.is_open()){
					...
					if ( it == list_of_files.begin())
						result_file << "Hamming wt  Fractional distance: ";
                    // Minimal output of plain result values, separated with a " "
                    result_file << item->ones << "  " << item->frd << ",";
...
int Hamming_Distance(struct Item *item)
{
    ...
    //bitwise XOR f1data and f2data to get the positions where bits differ
    for (i = 0; i < dsize; i++) {
...
int generateHelperData(volatile unsigned char *inputSecret,
        volatile unsigned char *sram, struct Item *item)
{
    int inputSize = item->input_length;
    int LRfactor = item->LR;
    char *helperfile = item->output_HD_name;
    ...
    // print to file
    FILE *ptr2;
    if ((ptr2 = fopen(helperfile, "wb")) == NULL)
    {
...
int Golay_encode(struct Item *item)
{
    ...
    FILE *fd1;
    unsigned int error = 0;
    unsigned char *sramData, *keydata;
    ...
    generateHelperData(keydata, sramData, item);
...
int recoverOriginalData(volatile unsigned char *sramData, int originalSecretSize, int len, struct Item *item)
{
    // Variables of this function
    int i = 0;
    int j = 0;
    int k = 0;
    int LRfactor = item->LR;
    long recd, recd2, temp, decoding_table[2048];
    unsigned long pos8 = 0, pos7 = 0 ,pos6 = 0 , pos5 = 0, pos4 = 0, pos3 = 0, pos2 = 0, pos1 = 0;
...
int Golay_decode(struct Item *item)
{
    ...
    recoverOriginalData(sram, item->input_Key_length, len, item);
error1:
    free(sram);
    fclose(fd);
     ...
     */
{
    ...
    // Check if more than one file is in the folder
    if(list_of_files.size() <= 1) return 17;
    ...
    for(it = list_of_files.begin();it != list_of_files.end(); ++it){
        ...
        //copy the first filename to input file
        strcpy(item->input_file_name, (*it).c_str());

        for(it2 = (it+1);it2 != list_of_files.end(); ++it2){

            //copy the next filenames to puf file and call Jaccard_Index function
            ...
            if (error == 0) {
                ...
                if (result_file.is_open()){
                    if(header){
                        result_file << "*******************************************************************************" << endl;
                        result_file << "                                                                               " << endl;
                        result_file << "                             Intra-Jaccard Index                               " << endl;
                        result_file << "                                                                               " << endl;
                        result_file << "                          Used Settings:                                       " << endl;
                        result_file << "                                                                               " << endl;
                        result_file << "                          Device folder: '" << item->input_path_name.at(0).c_str() <<"'" << endl;
                    ...
                    if(next_table){
                        if(max_file_length < 79)
                            white_spaces = string(69 - strlen((*it).c_str()), ' ');
                        else
                    ...
                    printf("unable to open result output filestream\n");
                    return 16;
                }
            }
            jindex = 0;
